...
LECTURES = [
    ...
    ("w3l3", 3, 3, 1, "Speed-dating защита + КТ1",
        ...
        []),
    ("w3l4", 3, 4, 1, "Evals своими руками: LLM-as-judge на нашем Qwen",
        "Ранняя eval-лаборатория на песочнице coffee-reviews-agent; судья на self-host Qwen "
        "(марж. стоимость ≈0), негативы и outcome-грейдер, калибровка против human, A/B + мини-ADR",
        ["Собрать eval-датасет (code/judge/human, негативы, outcome) и посчитать pass-rate",
         "Перевести судью на self-host Qwen и объяснить, почему eval теперь можно гонять на каждом коммите"],
    ...
    ("w5l2", 5, 2, 2, "Cost-of-AI ADR",
        ...
        ["cost-estimator"]),
    ("w6l1", 6, 1, 2, "Анатомия агента: точка входа в разработку",
        "Анатомия (core/loop/tools/memory/structured output), принципы (ReAct-loop), виды агентов "
        "и их плюсы/минусы, требования к данным: агент ≠ человек → canonical typed JSON (Data Plane)",
        ["Разложить агента на компоненты и выбрать паттерн (ReAct/planner/reflection/multi)",
         "Готовить данные для агента как canonical JSON (не сырьё) — агент воспринимает инфу не как человек"],
        ["agent-design-writer", "spec-reviewer"]),
    ...
    ("w9l3", 9, 3, 3, "RAG-pipeline, фреймворки, протоколы",
        ...
        ["rag-architect"]),
    ("w10l1", 10, 1, 3, "Observability: трейс траектории + cost-per-task",
        "Три сигнала наблюдаемости + трасса полной траектории агента (шаги/tool-calls); "
        "стоимость на задачу (а не на вызов), деградация, связка с cost-журналом FastMCP",
        ["Читать logs / metrics / traces и трассу траектории, объяснять сигналы",
         "Считать cost-per-task и ловить деградацию по трассе"],
        []),
    ...
    ("w11l2", 11, 2, 4, "Eval-датасет и прогон + prompt versioning",
        "30–50 примеров по СВОЕМУ проекту (навык из нед. 3), судья на self-host Qwen, "
        "trajectory/outcome-грейдеры, прогон по 2 cascade, метрики, версионирование промптов",
        ["Собрать eval-датасет по своему проекту и посчитать метрики (с outcome-грейдером)",
         "Версионировать промпты и выбрать cascade по цифрам (ADR)"],
        ["eval-generator", "adr-writer"]),
    ("w11l3", 11, 3, 4, "SFT на траекториях: дистилляция логов ape → LoRA Qwen",
        "Из успешных траекторий ape/Blackboard собираем SFT-датасет (вход+инструменты+исход по eval-грейдеру), "
        "LoRA fine-tune на нашем self-host Qwen, замер до/после ТЕМ ЖЕ eval; RL/GRPO — обзорно",
        ["Собрать SFT-датасет из траекторий, где исход прошёл eval-грейдер",
         "Запустить LoRA на self-host Qwen и доказать прирост по eval до/после"],
        ["eval-generator"]),
...
# ── Трек `managers`: «AI для менеджеров: основы работы с ИИ-агентами» (учебная программа, 9 тем,
# 48ч: 16 лек + 16 практ + 16 СУРС). Кортеж: code, week(=тема), seq, block, title, topic, outcomes[], skills[].
MANAGERS_BLOCK_NAMES = {1: "Основы и запросы", 2: "Данные, проверка, команда", 3: "Итог"}
MANAGERS_LECTURES = [
    ("mg1", 1, 1, 1, "Что такое ИИ и как он устроен",
        "LLM vs агент простыми словами; токен (единица объёма/стоимости), контекстное окно, природа "
        "галлюцинаций; где ИИ помогает менеджеру и где не заменяет. Принцип: уверенность модели ≠ правота",
        ["Различать LLM и агента; понимать токен, контекст, галлюцинацию на рабочем уровне",
         "Определять задачи, пригодные и непригодные для передачи ИИ"], []),
    ("mg2", 2, 1, 1, "Промпт: как ставить задачу ИИ",
        "Промпт = задание; качество ответа зависит от качества задания. Структура РЗФО (роль/задача/"
        "формат/ограничения); приёмы: примеры, уточняющие вопросы, пошаговое рассуждение, N вариантов",
        ["Составлять эффективный запрос по структуре РЗФО",
         "Усиливать промпт и видеть разницу до/после"], []),
    ("mg3", 3, 1, 1, "Скиллы: готовые методики",
        "Скилл = упакованная методика (правила+шаги). Подключение из портала; обзор скиллов под "
        "управленческие задачи (письма, интервью, критика идей, финсводки, оценка идей)",
        ["Подключать готовые скиллы и применять к рабочей задаче",
         "Понимать выигрыш скилла против «просто спросить»"],
        ["email-draft", "icp-interviewer", "devils-advocate", "finance-report", "idea-scorer"]),
    ("mg4", 4, 1, 1, "Свой скилл: упаковка экспертизы",
        "Повторяющийся процесс → скилл. Структура: когда применять · шаги (методика) · правила · "
        "Definition of Done. Составление без кода (в т.ч. с помощью ИИ); проверка на реальном случае",
        ["Составить свой скилл на рабочий процесс (условия/шаги/правила/DoD)",
         "Проверить скилл на реальном кейсе"], []),
    ("mg5", 5, 1, 2, "Данные, RAG и информационная безопасность",
        "ИИ работает только с переданным. Приложение документов, RAG на пальцах (база знаний→поиск→ответ "
        "со ссылкой). ИБ: куда уходят данные, что НЕЛЬЗЯ передавать (ПДн/тайна/доступы), обезличивание, "
        "личное правило проверки перед отправкой",
        ["Прикладывать документы и получать ответы с опорой на источники",
         "Определять недопустимые к передаче данные и обезличивать чувствительное"], []),
    ("mg6", 6, 1, 2, "Проверка результата и этика",
        "Галлюцинация как штатная особенность. Менеджерский минимум проверки (сверка фактов, правдоподобие, "
        "полнота), мини-оценка по критериям, второе мнение. Этика: bias, авторство/ответственность, прозрачность",
        ["Проверять результат по критериям и ловить галлюцинации",
         "Учитывать предвзятость и ответственность при решениях о людях"], []),
    ("mg7", 7, 1, 2, "Выбор уровня решения, экономика и несколько агентов",
        "Шкала сложности: простой запрос → со скиллом → с данными → несколько агентов → отказ от ИИ. "
        "Минимально достаточный уровень. Делегирование по ролям (когда оправдано). Экономика: факторы "
        "стоимости, контроль расходов. Вендор-грамотность (хранение данных, цена при масштабе, качество)",
        ["Выбирать минимально достаточный уровень решения и оценивать стоимость",
         "Делегировать задачу нескольким агентам, когда это оправдано"], []),
    ("mg8", 8, 1, 2, "Управление командой в эпоху ИИ",
        "Нормы использования ИИ в команде (допустимое/недопустимое, данные, ответственность, прозрачность). "
        "Оценка работы с ИИ по существу, а не по гладкости. Работа с опасениями сотрудников. План внедрения "
        "(пилоты с очевидной пользой, демонстрация, обратная связь, общая библиотека методик)",
        ["Сформировать нормы использования ИИ и оценивать работу по существу",
         "Составить план внедрения ИИ в своё подразделение"], []),
    ("mg9", 9, 1, 3, "Итоговая работа: мини-ассистент",
        "Комплекс под одну реальную задачу: РЗФО + скилл + данные (с ИБ) + критерии проверки + при "
        "необходимости делегирование. Подготовка и защита мини-ассистента (5 мин)",
        ["Собрать мини-ассистента под свою реальную задачу",
         "Защитить решение, обосновав выигрыш против разового обращения к ИИ"], []),
]
MANAGERS_PRACTICE = {
    1: "Определи задачи своей рабочей недели, пригодные и непригодные для передачи ИИ; сделай первый запрос через портал и оцени ответ и его стоимость.",
    5: "Приложи 3–5 рабочих документов, собери мини-базу знаний, задай 5 вопросов; составь личный список данных, недопустимых к передаче ИИ, и потренируй обезличивание.",
    8: "Набросай черновик норм использования ИИ для своей команды и план внедрения (пилотная задача с очевидной пользой).",
}
MANAGERS_ASSIGNMENTS = [
    (2, "Чек-лист: промпт", "Запрос по РЗФО под реальную задачу + сравнение ответа до/после улучшения (3 строки вывода).", "md", 10),
    (3, "Чек-лист: скилл", "Подключить подходящий скилл и применить к своей задаче; 3 строки — что скилл добавил.", "md", 10),
    (4, "Чек-лист: свой скилл", "Свой скилл на 1 процесс: когда применять · 3–6 шагов · 2–3 правила · DoD; проверить на кейсе.", "md", 10),
    (5, "Чек-лист: данные и ИБ", "Мини-база знаний из 3–5 документов + личный список недопустимых к передаче данных; пример обезличивания.", "md", 10),
    (6, "Чек-лист: проверка", "Оценить 3 ответа ИИ по своим критериям (критерий→да/нет); найти 1 галлюцинацию; отметить этический риск.", "md", 10),
    (7, "Чек-лист: уровень и агенты", "Для 3 задач выбрать минимально достаточный уровень (запрос/скилл/данные/агенты/отказ); одну разложить на роли.", "md", 10),
    (8, "Политика команды", "Черновик норм использования ИИ для команды + план внедрения (пилотная задача с очевидной пользой).", "md", 10),
    (9, "Зачёт: мини-ассистент", "Связка РЗФО+скилл+данные(с ИБ)+критерии проверки под реальную задачу; защита 5 мин.", "md", 20),
]
...
PRACTICE = {
    ...
    3: "Составь eval-набор на 10 кейсов для агента своей идеи (даже без кода): вход→ожидание, для каждого тип грейдера (code/judge/human) и почему. Судью в прогоне держи на профиле code (наш Qwen). Сдай eval/<проект>.jsonl ссылкой на коммит.",
    ...
    6: "Собери минимального агента pet-проекта (см. docs/lectures/dz-agent-anatomy.md): анатомия из 8 компонентов, выбранный паттерн, loop с условием остановки, ≥2 tools (один — data_query), structured output и 1 Data-recipe источник→canonical JSON. В NOTES.md 5 строк «где агент ≠ человек в моих данных».",
    ...
    11: "Отбери из логов ape 15–30 успешных траекторий (исход прошёл твой eval-грейдер) и оформи их как SFT-датасет (вход+инструменты+ответ). Опиши в 5 строках, какой прирост по eval ждёшь от LoRA и как замеришь до/после.",
...
async def ensure() -> None:
    ...
    async with db._conn() as c:  # noqa: SLF001
        ...
        # Мультикурсовость: трек лекции/задания (engineer — основной, managers — обзорный).
        await c.execute("ALTER TABLE lectures ADD COLUMN IF NOT EXISTS track TEXT DEFAULT 'engineer'")
        await c.execute("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS track TEXT DEFAULT 'engineer'")
        ...
        # ── трек managers: пере-сев при рассинхроне с кодом (число != len или нет mg9).
        # Только managers-строки; engineer не трогаем. Лектор managers-план в портале не редактирует
        # вручную, поэтому пересев безопасен (контент — источник в коде).
        mlec = (await (await c.execute(
            "SELECT COUNT(*) FROM lectures WHERE track='managers'")).fetchone())[0]
        has_mg9 = (await (await c.execute(
            "SELECT COUNT(*) FROM lectures WHERE track='managers' AND code='mg9'")).fetchone())[0]
        if mlec != len(MANAGERS_LECTURES) or not has_mg9:
            await c.execute("DELETE FROM lectures WHERE track='managers'")
            base = (await (await c.execute("SELECT COALESCE(MAX(position),0) FROM lectures")).fetchone())[0]
            for i, (code, wk, seq, bl, t, tp, outs, sk) in enumerate(MANAGERS_LECTURES, 1):
                await c.execute(
                    "INSERT INTO lectures(code,week,seq,block,title,topic,outcomes,skills,practice,position,track) "
                    "VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'managers')",
                    (code, wk, seq, bl, t, tp, "|".join(outs), ",".join(sk),
                     MANAGERS_PRACTICE.get(wk, ""), base + i))
        masg = (await (await c.execute(
            "SELECT COUNT(*) FROM assignments WHERE track='managers'")).fetchone())[0]
        if masg != len(MANAGERS_ASSIGNMENTS):
            await c.execute("DELETE FROM assignments WHERE track='managers'")
            for wk, t, d, f, ms in MANAGERS_ASSIGNMENTS:
                await c.execute(
                    "INSERT INTO assignments(week,title,description,fmt,max_score,position,track) "
                    "VALUES(%s,%s,%s,%s,%s,%s,'managers')", (wk, t, d, f, ms, wk))
