...
# ── Трек `managers`: обзорный курс для менеджеров (docs/course-managers-overview.md).
# 8 коротких модулей. Тот же кортеж: code, week(=модуль), seq, block, title, topic, outcomes[], skills[].
MANAGERS_BLOCK_NAMES = {1: "Основы", 2: "Применение и итог"}
MANAGERS_LECTURES = [
    ("mg1", 1, 1, 1, "Что такое ИИ-агент и зачем менеджеру",
        "LLM vs агент простыми словами, что умеет и что нет, где помогает менеджеру; уверенность ≠ правота",
        ["Отличать один вызов LLM от агента и понимать, где ИИ помогает, а где нет",
         "Сделать первый запрос и увидеть, что у ИИ есть стоимость"], []),
    ("mg2", 2, 1, 1, "Как разговаривать с ИИ: промпт",
        "Промпт = задание. Формула РЗФО (роль/задача/формат/ограничения), хороший vs плохой, приёмы",
        ["Составлять промпт по РЗФО под свою рабочую задачу",
         "Видеть разницу ответа до и после улучшения промпта"], []),
    ("mg3", 3, 1, 1, "Скиллы: готовые методики",
        "Что такое скилл, зачем, как подключить из интерфейса; скиллы под менеджера",
        ["Подключать готовые скиллы из UI и применять к задаче",
         "Понимать, что скилл добавляет по сравнению с «просто спросить»"],
        ["email-draft", "icp-interviewer", "devils-advocate"]),
    ("mg4", 4, 1, 1, "Свой скилл: упаковать экспертизу",
        "Из повторяющейся задачи → скилл (когда применять · шаги · правила · DoD), без кода",
        ["Составить простой свой скилл на рабочий процесс",
         "Проверить скилл на реальном кейсе"], []),
    ("mg5", 5, 1, 2, "Данные для ИИ: он не человек",
        "Почему ИИ нужны чистые данные, вложения файлов, RAG на пальцах (база знаний)",
        ["Прикладывать файлы и получать ответы по документам, а не «из головы»",
         "Понимать, что делает данные пригодными для ИИ"], []),
    ("mg6", 6, 1, 2, "Проверка результата: не верь на слово",
        "Галлюцинации, как проверять (цитаты/здравый смысл/полнота), мини-оценка по критериям",
        ["Проверять вывод ИИ и ловить галлюцинации",
         "Сравнивать варианты по проверяемым критериям, а не на глаз"], []),
    ("mg7", 7, 1, 2, "Несколько агентов и делегирование",
        "Разбить большую задачу на роли и запустить агентов передачей задач; когда это нужно",
        ["Разложить рабочую задачу на роли и запустить несколько агентов",
         "Понимать, когда дробить задачу, а когда хватит одного агента"], []),
    ("mg8", 8, 1, 2, "Итог: свой мини-ассистент",
        "Связать промпт + скилл + данные + проверку под одну реальную задачу; защита",
        ["Собрать рабочего мини-ассистента под свою задачу",
         "Показать, чем результат лучше «просто спросить у ИИ»"], []),
]
MANAGERS_PRACTICE = {
    1: "Выпиши 3 задачи своей недели, которые можно отдать ИИ, и 1, которую отдавать не стоит (почему).",
    5: "Приложи 3–5 своих документов и задай 5 рабочих вопросов; отметь случай, где без документа ИИ ошибся бы.",
}
MANAGERS_ASSIGNMENTS = [
    (2, "Составить промпт", "Промпт под реальную задачу по РЗФО + сравнение ответа до/после (3 строки вывода).", "md", 10),
    (3, "Подключить скилл", "Подключить подходящий скилл и применить к своей задаче; 3 строки — что скилл добавил.", "md", 10),
    (4, "Составить свой скилл", "Свой скилл на 1 процесс: когда применять · 3–6 шагов · 2–3 правила · DoD; проверить на кейсе.", "md", 10),
    (6, "Проверка результата", "Оценить 3 ответа ИИ по своим критериям (таблица критерий→да/нет); найти 1 галлюцинацию.", "md", 10),
    (7, "Несколько агентов", "Разложить рабочую задачу на 2–3 роли, запустить агентов; вывод о пользе делегирования.", "md", 10),
    (8, "Мини-ассистент", "Связка промпт+скилл+данные+проверка под реальную задачу + защита 5 мин.", "md", 20),
]
...
async def ensure() -> None:
    ...
    async with db._conn() as c:  # noqa: SLF001
        ...
        # ── трек managers: сид отдельно (по счётчику track='managers'), доедет и на прод ──
