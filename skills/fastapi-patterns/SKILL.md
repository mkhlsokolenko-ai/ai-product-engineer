...
Ты пишешь FastAPI-код продакшен-уровня: раскладка проекта, схемы Pydantic v2, dependency injection, async-паттерны, аутентификация/авторизация, транзакционный сервисный слой и тесты. Цель — тонкие роуты, толстый сервис, чистый контракт наружу и никаких утечек данных через ответы.

## Когда применять
- Реализуешь эндпоинты по уже согласованному контракту (`api-design`).
- Заводишь новый FastAPI-сервис: раскладка, конфиг, DI, lifespan.
- Добавляешь аутентификацию/авторизацию или транзакционный сервисный слой.
- Пишешь тесты API через httpx/pytest.
...
## Definition of Done
- [ ] Раскладка разделена: роуты тонкие, бизнес-логика в сервисном слое, схемы отдельно от ORM-моделей.
- [ ] На КАЖДОМ эндпоинте объявлен типизированный `response_model` — поля наружу отдаются явно, ничего лишнего (ПДн, хэши, внутренние флаги) не утекает.
- [ ] Аутентификация проверяется на входе через зависимость (`get_current_user`), авторизация отделена от аутентификации (`401` vs `403`).
- [ ] Мутации обёрнуты в транзакции сервисного слоя; уникальность — на уровне БД-констрейнта, не гонки в приложении.
- [ ] В async-роутах только async-обращения к БД (никаких синхронных вызовов, блокирующих event loop).
- [ ] Пагинация с детерминированной сортировкой и ограниченным сверху `limit`.
- [ ] Секреты и конфиг — через `pydantic-settings`/`.env`, не хардкод; `.env` не в репозитории.
- [ ] Есть тесты (httpx + pytest): happy path, авторизация (403), валидация (422).

## Анти-паттерны
- Бизнес-логика внутри хендлера роута вместо сервисного слоя.
- Синхронные вызовы БД в async-роуте — блокируют event loop.
- Отдача ORM-модели без `response_model` — риск утечки полей (пароль-хэш, ПДн, внутренние флаги).
- Проверка уникальности предвыборкой в приложении вместо атомарного БД-констрейнта — гонка.
- Логирование токенов, паролей, ПДн в открытом виде.
- Слияние аутентификации и авторизации в одну проверку — теряется точный статус (`401` vs `403`).
- Секреты в коде/образе вместо переменных окружения.

## Безопасность
- `mode=read`, `egress=internal`, `cite=False`.
- Навык проектирует/пишет код внутри периметра, наружу сам не ходит (единственная egress-точка сервиса — LLM-прокси, не этот код).
- **Authz на входе:** авторизация валидируется один раз на входе (зависимость с кэшированием токена), а не на каждом внутреннем вызове.
- **Не логировать чувствительное:** секреты (`secret_key`, токены), пароли и ПДн не попадают в логи/трейсы.
- **`response_model` как гейт утечки:** типизированный ответ гарантирует, что наружу не уйдут поля вроде `hashed_password` или служебных признаков.

## Интеграция
- `data_query`: на входе сверься с существующими контрактами и моделями данных (`api-design`), чтобы схемы/именование/ошибки были консистентны.
- `remember(criticality)`: пометь критичность эндпоинтов (мутации, egress-точки, доступ к ПДн) — вход для гейта конверта; обычно средняя-высокая критичность.
- `handoff`: контракт получай из `api-design`; решения по версионированию/совместимости фиксируй через `adr-writer`; тесты передавай/дополняй через `test-writer`.
